...
if __name__ == '__main__':
    ...
    #this was renamed to mandelbrot_dask
    #result = mandelbrot_parallel(N, x_min, x_max, y_min, y_max, max_iter,n_workers=4) #actual run
    #result = mandelbrot_dask(N, x_min, x_max, y_min, y_max, max_iter, n_chunks=32) #actual run

    client = Client("tcp://10.92.1.38:8786")
...
# Chunk-count sweep (M2): one Pool per config
    ...
    #     print(f"{n_chunks:4d} chunks {t_par:.3f}s {t_serial/t_par:.1f}x LIF={lif:.2f}")
        #All above was for when it was mandelbrot_parallel, now it's mandelbrot_dask also removed , n_workers=n_workers and pool=pool

# Print table header
    print(f"{'n_chunks':>8} | {'time (s)':>8} | {'vs 1x':>6} | {'speedup':>8} | {'LIF':>6}")
    print("-" * 50)
    ...
    for mult in [1, 2, 4, 8, 16]:
        ...
        times = []
        for _ in range(3):
            mandelbrot_dask(N, x_min, x_max, y_min, y_max, max_iter=10, n_chunks=n_chunks)
            t0 = time.perf_counter()
            mandelbrot_dask(N, x_min, x_max, y_min, y_max, max_iter, n_chunks=n_chunks)
            times.append(time.perf_counter() - t0)
        ...
        vs_1x = t_par / t_serial   # normalized wall time (1.0 = same as serial)
        lif = n_workers * t_par / t_serial - 1

        print(f"{n_chunks:>8} | {t_par:>8.3f} | {vs_1x:>6.2f} | {speedup:>8.1f}x | {lif:>6.2f}")
    ...
    print(f"\nOptimal: n_chunks={best[0]}, t_min={best[1]:.3f}s, LIF={best[3]:.2f}")

    # Plot
    n_chunks_vals = [r[0] for r in results]
    times_vals    = [r[1] for r in results]

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.plot(n_chunks_vals, times_vals, marker='o', linewidth=2)
    ax.axhline(t_serial, color='red', linestyle='--', label=f'Serial baseline ({t_serial:.3f}s)')
    ax.set_xscale('log', base=2)
    ax.set_xlabel('n_chunks')
    ax.set_ylabel('Wall time (s)')
    ax.set_title('Dask chunk sweep — wall time vs n_chunks')
    ax.legend()
